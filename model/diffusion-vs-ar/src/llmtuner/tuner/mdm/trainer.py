...
# Sudoku extension modules (optional)
try:
    # Add project root to path to find sudoku_ext
    import sys
    import os
    # Get the project root directory (diffusion-vs-ar)
    ...
    from sudoku_ext.guided_topk import guided_topk_selection
    SUDOKU_EXT_AVAILABLE = True
except ImportError as e:
    SUDOKU_EXT_AVAILABLE = False
    get_config = None
    ConstraintEnergy = None
    GuidedTopK = None
    SudokuConfig = None
    guided_topk_selection = None
    logger.warning(f"Failed to import sudoku_ext modules: {e}")
...
class CustomDiffusionTrainer(Trainer):
    ...
    def inner_forward(
        ...
        # Add Sudoku constraint energy if enabled
        if SUDOKU_EXT_AVAILABLE:
            config = get_config()
            if config.enable_constraint_energy:
                # Debug logging
                if config.verbose:
                    logger.info(f"[SUDOKU_EXT] Constraint energy enabled: lambda_e={config.lambda_e}, energy_type={config.energy_type}, t_e_ratio={config.t_e_ratio}")
                print(f"[SUDOKU_DEBUG] Constraint energy enabled: lambda_e={config.lambda_e}, energy_type={config.energy_type}")
                try:
                    # Compute constraint energy using full logits
                    # Energy module assumes digits are the last 9 tokens
                    energy_module = ConstraintEnergy(config)
                    total_loss, energy_metrics = energy_module.add_to_loss(
                        logits, loss, t, num_timesteps, mask=None
                    )

                    # Update loss
                    loss = total_loss

                    # Log energy metrics if verbose and we have state
                    if config.verbose and hasattr(self, 'state') and hasattr(self.state, 'global_step'):
                        if self.state.global_step % 100 == 0:
                            logger.info(f"Step {self.state.global_step}: "
                                      f"total_loss={loss.item():.4f}, "
                                      f"base_loss={energy_metrics.get('base_loss', 0):.4f}, "
                                      f"energy={energy_metrics.get('energy', 0):.4f}, "
                                      f"energy_weighted={energy_metrics.get('energy_weighted', 0):.4f}")
                except Exception as e:
                    logger.warning(f"Failed to compute constraint energy: {e}")
                    # Continue with original loss
            ...
            with torch.no_grad():
                ...
                if t > 0:
                    if self.diff_args.topk_decoding:
                        # Check if guided topk is enabled
                        use_guided_topk = False
                        if SUDOKU_EXT_AVAILABLE:
                            config = get_config()
                            if config.enable_guided_topk:
                                use_guided_topk = True
                                if config.verbose:
                                    logger.info(f"[SUDOKU_EXT] Guided topk enabled: alpha_u={config.alpha_u}, beta_c={config.beta_c}, k_min={config.k_min}, k_max={config.k_max}")
                                print(f"[SUDOKU_DEBUG] Guided topk enabled: alpha_u={config.alpha_u}, beta_c={config.beta_c}")

                        if use_guided_topk:
                            # Use guided topk selection
                            try:
                                # Compute guided topk mask
                                indices_mask, _ = guided_topk_selection(
                                    logits=logits,
                                    current_tokens=x0,
                                    maskable_mask=maskable_mask,
                                    t=t,
                                    T=self.diff_args.diffusion_steps,
                                    config=config
                                )

                                # indices_mask is True for positions to update
                                # Convert to masked_to_noise (positions to re-mask)
                                masked_to_noise = indices_mask

                                # Apply mask to x0
                                if isinstance(self.tokenizer.mask_token_id, torch.Tensor):
                                    xt = x0.masked_scatter(masked_to_noise, self.tokenizer.mask_token_id[masked_to_noise])
                                else:
                                    xt = x0.masked_fill(masked_to_noise, self.tokenizer.mask_token_id)

                            except Exception as e:
                                logger.warning(f"Guided topk failed, falling back to standard topk: {e}")
                                # Fall back to standard topk decoding
                                xt = topk_decoding(
                                    x0,
                                    x0_scores,
